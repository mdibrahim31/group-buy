...
export const SubAdminPanelModal: React.FC<SubAdminPanelModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900 flex flex-col font-sans">
      {/* Sleek Dark Top Header */}
      <div className="bg-stone-900 border-b border-stone-800 text-white px-6 py-4 flex items-center justify-between shrink-0 sticky top-0 z-50 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-sm tracking-tight sm:text-base">সাব-অ্যাডমিন ড্যাশবোর্ড (Sub-Admin Dashboard)</h3>
              <span className="text-[10px] bg-emerald-950 text-emerald-300 border border-emerald-700 px-2 py-0.5 rounded-full font-mono font-bold flex items-center gap-1 shrink-0">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                লাইভ
              </span>
            </div>
            <p className="text-[10px] text-stone-400">বান্ডিল পোস্ট এবং প্রোডাক্ট কো-অর্ডিনেশন গেটওয়ে</p>
          </div>
        </div>
        <button
          onClick={onClose}
          className="px-3 py-1.5 bg-stone-800 hover:bg-rose-700 text-stone-300 hover:text-white rounded-xl transition-all cursor-pointer flex items-center gap-1.5 font-bold text-xs border border-stone-700 hover:border-rose-600"
      ...
      <div className="flex-1 p-4 sm:p-6 md:p-8 bg-stone-950 text-stone-100 flex flex-col">
        {notification && (
          <div className="max-w-4xl mx-auto w-full bg-emerald-600 text-white p-3.5 rounded-2xl mb-5 text-xs font-bold flex items-center gap-2 animate-bounce shadow-lg">
        ...
        {!isAccessGranted ? (
          <div className="max-w-md mx-auto py-16 text-center w-full my-auto">
            <div className="w-16 h-16 bg-amber-500/10 border border-amber-500/30 text-amber-500 rounded-3xl flex items-center justify-center mx-auto mb-4 shadow-sm">
              <Lock className="w-8 h-8 animate-bounce" />
            </div>
            <h4 className="text-base font-black text-white mb-2 sm:text-lg">সাব-অ্যাডমিন সিক্রেট ভেরিফিকেশন</h4>
            <p className="text-xs text-stone-400 mb-6 leading-relaxed max-w-sm mx-auto">
            ...
            <form onSubmit={handleVerifyAccessCode} className="space-y-4">
              <input
                ...
                className="w-full text-center px-4 py-3 bg-stone-900 border border-stone-700 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 text-white shadow-2xs placeholder-stone-500"
              />
              {accessError && <p className="text-xs text-rose-500 font-bold mt-2">{accessError}</p>}
              <button
                type="submit"
        ...
        !currentSubAdmin ? (
          <div className="max-w-md mx-auto py-10 w-full my-auto">
            {/* Tab Selector */}
            <div className="flex bg-stone-900 p-1.5 rounded-2xl mb-6 border border-stone-800">
              <button
                onClick={() => {
                  setAuthMode('login');
                  setAuthError(null);
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-xl cursor-pointer transition-all ${
                  authMode === 'login' ? 'bg-stone-800 text-white shadow-sm' : 'text-stone-500 hover:text-stone-300'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-xl cursor-pointer transition-all ${
                  authMode === 'register' ? 'bg-stone-800 text-white shadow-sm' : 'text-stone-500 hover:text-stone-300'
                }`}
              >
            ...
            {authError && (
              <div className="bg-rose-950/40 border border-rose-900/50 text-rose-300 p-3.5 rounded-2xl text-xs font-semibold mb-4 flex items-start gap-2 animate-pulse">
            ...
            {authSuccess && (
              <div className="bg-emerald-950/40 border border-emerald-900/50 text-emerald-300 p-3.5 rounded-2xl text-xs font-semibold mb-4 flex items-start gap-2">
            ...
            {authMode === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-4 bg-stone-900 border border-stone-800 p-6 rounded-3xl shadow-lg">
                <div>
                  <label className="block text-xs font-bold text-stone-400 mb-1.5">ফোন নম্বর:</label>
                  <input
                    type="text"
                    placeholder="017XXXXXXXX"
                    value={phone}
                    onChange={(e) => setPhone(e.target.value)}
                    className="w-full px-4 py-2.5 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    required
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-stone-400 mb-1.5">পাসওয়ার্ড:</label>
                  <input
                    type="password"
                    placeholder="••••••••"
                    value={password}
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all cursor-pointer shadow-sm mt-2"
                >
                  লগইন করুন
                </button>
              </form>
            ) : (
              <form onSubmit={handleRegister} className="space-y-4 bg-stone-900 border border-stone-800 p-6 rounded-3xl shadow-lg">
                <div>
                  <label className="block text-xs font-bold text-stone-400 mb-1.5">সাব-অ্যাডমিন নাম (Full Name):</label>
                  <input
                    type="text"
          ...
          <div className="max-w-6xl mx-auto w-full space-y-6">
            {/* Profile Card Banner */}
            <div className="bg-stone-900 border border-stone-800 rounded-3xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-emerald-600 text-white font-black text-lg rounded-2xl flex items-center justify-center shadow-md">
                  {currentSubAdmin.fullName.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="text-sm font-black text-white flex items-center gap-2">
                    <span>স্বাগতম, {currentSubAdmin.fullName}!</span>
                    <span className="bg-emerald-950 text-emerald-300 border border-emerald-800 text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                      <ShieldCheck className="w-3 h-3 text-emerald-400" />
                      <span>সাব-অ্যাডমিন</span>
                    </span>
                  </h4>
                  <p className="text-[10px] text-stone-400 mt-0.5">মোবাইল: {currentSubAdmin.phone} | আইডি: {currentSubAdmin.id}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <button
                  ...
                  className="p-2.5 bg-stone-800 text-stone-400 hover:text-white hover:bg-stone-700 rounded-xl transition-all cursor-pointer border border-stone-700"
                >
                  <Lock className="w-4 h-4" />
                </button>
                <button
                  onClick={subAdminLogout}
                  className="px-4 py-2.5 bg-rose-950/60 text-rose-300 hover:bg-rose-900 hover:text-white border border-rose-900/40 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
                >
            ...
            {dashboardView === 'list' ? (
              <div className="space-y-5">
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 bg-stone-900 border border-stone-800 p-4 rounded-2xl shadow-sm">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
                    ...
                    {/* Owner Filter Toggle */}
                    <div className="flex bg-stone-950 border border-stone-800 p-1 rounded-xl items-center gap-1 shrink-0 mt-2 sm:mt-0">
                      <button
                ...
                {myProducts.length === 0 ? (
                  <div className="text-center py-20 bg-stone-900 border border-stone-800 rounded-3xl p-6">
                    <Sparkles className="w-12 h-12 text-emerald-500 mx-auto mb-3 animate-pulse" />
                    <h5 className="text-sm font-bold text-white mb-1">কোনো আপলোড করা বান্ডিল পাওয়া যায়নি</h5>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {myProducts.map(prod => {
                      ...
                      return (
                        <div key={prod.id} className="bg-stone-900 border border-stone-800 rounded-3xl p-4 flex gap-4 shadow-sm hover:border-emerald-600 transition-all">
                          {/* Image */}
                          <div className="relative shrink-0">
                            <img
                              src={prod.imageUrl}
                              alt={prod.title}
                              className="w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-2xl border border-stone-800"
                              onError={(e) => {
                                (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=200';
                              }}
                            />
                            <span className="absolute bottom-1 right-1 bg-stone-950/90 backdrop-blur-xs text-emerald-400 border border-stone-800 text-[8px] font-bold px-2 py-0.5 rounded-md uppercase">
                              {prod.category}
                            </span>
                          </div>
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-between">
                            <div>
                              <div className="flex items-center justify-between gap-1.5 mb-1.5">
                                <span className="text-[9px] font-black bg-stone-950 border border-stone-800 text-stone-300 px-2 py-0.5 rounded-md">
                                  বান্ডিল সাইজ: {prod.bundleSize}টি
                                </span>
                                {prod.createdBySubAdminId === currentSubAdmin.id && (
                                  <button
                                    onClick={() => handleOpenEditForm(prod)}
                                    className="px-2.5 py-1 bg-stone-800 hover:bg-stone-700 border border-stone-700 rounded-md text-[10px] font-bold text-white flex items-center gap-1 transition-colors cursor-pointer"
                                  >
                              ...
                              <h4 className="text-xs sm:text-sm font-black text-white truncate">{prod.title}</h4>
                              <div className="text-[11px] text-stone-400 mt-1 space-y-0.5">
                                <p>গ্রুপ রেট: <strong className="text-emerald-400">৳{prod.groupPrice}</strong> | হোলসেল: ৳{prod.wholesalePrice}</p>
                            ...
                            <div className="mt-2.5 pt-2 border-t border-stone-800">
                              <span className="text-[10px] font-extrabold text-stone-300">সক্রিয় ব্যাচ প্রগ্রেস:</span>
                              {prodBundles.length === 0 ? (
                                <p className="text-[9px] text-stone-500 italic">কোনো সক্রিয় ব্যাচ নেই</p>
                              ) : (
                                <div className="space-y-2 mt-1">
                                  {prodBundles.map(b => (
                                    <div key={b.id} className="bg-stone-950 border border-stone-800 rounded-xl p-2.5">
                                      <div className="flex items-center justify-between text-[10px] mb-1">
                                        <span className="font-bold text-stone-400">ব্যাচ #{b.batchNumber}</span>
                                        <span className="text-emerald-400 font-extrabold">{b.filledSlots}/{b.totalSlots} স্লট বুকড</span>
                                      </div>
                                          ...
                                          return (
                                            <span
                                              ...
                                              className={`text-[8px] font-bold px-1.5 py-0.5 rounded border ${
                                                isBooked
                                                  ? 'bg-emerald-950/60 border-emerald-900/50 text-emerald-400 font-extrabold'
                                                  : 'bg-stone-900 border-dashed border-stone-800 text-stone-500'
                                              }`}
              ...
              /* View 2: Form to Add/Edit Product - HUBUHU SAME AS MAIN ADMIN FORM */
              <div className="max-w-2xl mx-auto bg-stone-900 border border-stone-800 rounded-2xl p-5 sm:p-6 mb-12 shadow-md">
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-stone-800">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 text-[10px] font-bold bg-stone-800 hover:bg-stone-700 text-stone-300 rounded-lg transition-colors cursor-pointer"
                  >
                    বাতিল
                  </button>
                </div>
                ...
                {formError && (
                  <div className="mb-4 p-3 bg-rose-950/40 border border-rose-900/50 text-rose-300 text-xs font-semibold flex items-center gap-2 rounded-xl animate-pulse">
                ...
                <form onSubmit={dashboardView === 'add' ? handleAddProductSubmit : handleEditProductSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none placeholder-stone-600"
                      required
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs font-medium text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none cursor-pointer"
                      >
                        {categories.map((cat) => (
                    ...
                    <div>
                      ...
                      <div className="w-full px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs font-bold text-white flex items-center justify-between h-9">
                        <span className="text-emerald-400 font-extrabold text-sm">{calculatedTotalPcs} পিস</span>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[10px] text-stone-500">অনলাইন ছবির সরাসরি লিংক দিন</span>
                    </div>
                    <input
                      type="url"
                      ...
                      className="w-full px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none font-mono placeholder-stone-700"
                      required
                    />
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5 items-center">
                        ...
                        {imagePresets.map((preset, idx) => (
                          <button
                            ...
                            className="px-2 py-0.5 bg-stone-800 hover:bg-stone-700 border border-stone-700 rounded text-[10px] text-stone-300 transition-colors cursor-pointer"
                          >
                            + {preset.label}
                          </button>
                        ))}
                    ...
                    {/* Image Preview */}
                    {prodImageUrl && (
                      <div className="mt-2.5 flex items-center gap-3 p-2 bg-stone-950 rounded-xl border border-stone-800">
                        <img
                          src={prodImageUrl}
                          alt="Preview"
                        ...
                        />
                        <span className="text-[11px] text-emerald-400 font-semibold">প্রধান ছবি প্রিভিউ সফল</span>
                      </div>
                    )}
                  </div>

                  {/* 5 Additional Image Links (Sequential vertical scroll) */}
                  <div className="p-3.5 bg-stone-950 border border-stone-800 rounded-2xl space-y-2.5">
                      ...
                      {prodAdditionalImages.map((imgUrl, imgIdx) => (
                        <div key={imgIdx} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-1.5 bg-stone-900 border border-stone-800 rounded-lg text-xs font-mono text-white focus:ring-1 focus:ring-emerald-500 focus:outline-none placeholder-stone-700"
                          />
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-6 pr-2 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none font-bold placeholder-stone-700"
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-emerald-400 mb-1.5">
                        গ্রুপ বাই মূল্য/পিস *
                      </label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-6 pr-2 py-2 bg-emerald-950/20 border border-emerald-900/50 rounded-xl text-xs text-emerald-300 focus:ring-2 focus:ring-emerald-500 focus:outline-none font-bold placeholder-stone-700"
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-6 pr-2 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none font-medium placeholder-stone-700"
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 bg-stone-950 border border-stone-800 rounded-2xl space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 pb-2 border-b border-stone-800">
                      <div>
                      ...
                      <span className="text-xs font-bold bg-emerald-950/80 text-emerald-400 px-2.5 py-1 rounded-lg border border-emerald-800 shrink-0 self-start sm:self-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                      {sizeConfigs.map((sc) => (
                        <div
                          key={sc.id}
                          className="p-2 bg-stone-900 border border-stone-800 rounded-xl flex items-center justify-between gap-2 shadow-2xs"
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <input
                              ...
                              className="w-full px-2 py-1 bg-stone-950 border border-stone-800 rounded-lg text-xs font-bold text-white focus:bg-stone-900 focus:ring-1 focus:ring-emerald-500 focus:outline-none placeholder-stone-700"
                              required
                            />
                          </div>

                          <div className="w-24 shrink-0">
                            <label className="block text-[10px] text-stone-500 font-semibold mb-0.5 text-center">
                              পিস (Qty)
                            </label>
                            <div className="flex items-center border border-stone-800 rounded-lg overflow-hidden bg-stone-950">
                              <button
                                ...
                                className="px-2 py-1 text-xs font-bold text-stone-400 hover:bg-stone-800 cursor-pointer"
                              >
                                -
                              </button>
                              <input
                                ...
                                className="w-full text-center text-xs font-bold bg-transparent text-white focus:outline-none py-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                required
                              />
                              <button
                                type="button"
                          ...
                          {sizeConfigs.length > 1 && (
                            <button
                              ...
                              className="p-1.5 text-stone-500 hover:text-rose-500 hover:bg-rose-950/30 rounded-lg transition-colors cursor-pointer mt-3 shrink-0"
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-emerald-400 hover:text-white bg-emerald-950/40 hover:bg-emerald-900/40 border border-emerald-900/50 px-3 py-1.5 rounded-xl transition-colors cursor-pointer flex items-center gap-1.5 w-max"
                    >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 p-3 bg-stone-950 border border-stone-800 rounded-2xl min-h-[90px] max-h-[160px] overflow-y-auto">
                        {appColors.map((col) => {
                        ...
                        {appColors.length === 0 && (
                          <p className="text-[11px] text-stone-500 font-semibold italic">কোনো কালার পাওয়া যায়নি।</p>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2.5 bg-emerald-950/20 border border-emerald-900/50 rounded-xl text-xs text-emerald-300 focus:ring-2 focus:ring-emerald-500 focus:outline-none font-bold cursor-pointer"
                      >
                        {prodSelectedColors.map((color) => (
                          <option key={color} value={color} className="bg-stone-900 text-white">
                            {color}
                          </option>
                        ))}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2 bg-stone-950 border border-stone-800 rounded-xl text-xs text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none placeholder-stone-700"
                      required
                    />
                  </div>
